-- Add metadata to integrations in get_all_credentials response.
-- The integrations.metadata column exists; the function was not including it.
...
BEGIN
  ...
  IF p_include_secrets THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object('id', id, 'platformName', "platformName", 'isConnected', "isConnected", 'credentials', credentials, 'metadata', metadata, 'createdAt', "createdAt", 'updatedAt', "updatedAt")), '[]'::jsonb)
